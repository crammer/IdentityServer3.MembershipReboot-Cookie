...
namespace IdentityServer3.MembershipReboot
{
        ...
        where TAccount : UserAccount
    {
        ...
        public MembershipRebootUserService(IHttpContextAccessor httpContextAccessor)
        {
        ...
        protected virtual bool ValidateLocalCredentials(string username, string password, SignInMessage message, out TAccount account)
        {
            ...
            var membership = userAccountService.Authenticate(tenant, username, password, out account);
            var status = "Login Successful";
            if (!membership)
            {
                status = "Login Successful";
            }
            var Audit = new AuditHandler(HttpContextAccessor);
            Audit.LogAccess(username, password, status);
            return membership;
        }
